...
class WeatherStates(StatesGroup):
    ...
    waiting_for_intermediate = State()  # Ожидание промежуточной точки
    waiting_for_interval = State()      # Ожидание выбора интервала
...
def get_route_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(
                text="➕ Добавить точку маршрута", callback_data="add_point"
            )
        ],
        [
            InlineKeyboardButton(
                text="✅ Завершить маршрут", callback_data="finish_route"
            )
        ],
    ]
...
def get_interval_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(text="1 день", callback_data="interval_3"),
...
async def command_weather(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "🌍 Укажите начальную точку маршрута\n" "(Например: Москва)"
    )
...
async def process_end_point(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "🛣 Маршрут почти готов! Хотите добавить промежуточные точки?",
...
async def process_intermediate_point(
...
async def process_interval(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    route_info += (
        ...
        f"\n📅 Прогноз будет составлен "
        f"на {interval} дня(дней)"
    )
    ...
    await callback.message.answer(
        ...
        + get_weather_info(data, interval, get_cached=DEBUG)
    )
